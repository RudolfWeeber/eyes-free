...
public class TTSService extends Service implements OnCompletionListener {
   ...
   * package that contains the resource and the resID of the resource within
   * that package.
   */
  ...
  private final RemoteCallbackList<ITtsCallbackBeta> mCallbacks =
      new RemoteCallbackList<ITtsCallbackBeta>();
  ...
  private boolean isDefaultEnforced() {
    return PreferenceManager.getDefaultSharedPreferences(this).getBoolean("override_pref", false);
  ...
  private String getDefaultLanguage() {
    String defaultLang =
        PreferenceManager.getDefaultSharedPreferences(this).getString("lang_pref", "");
  ...
  private int setLanguage(String callingApp, String lang, String country, String variant) {
    ...
    try {
      if (isDefaultEnforced()) {
        res =
            sNativeSynth.setLanguage(getDefaultLanguage(), getDefaultCountry(),
   ...
   * @param queueMode TextToSpeech.TTS_QUEUE_FLUSH for no queue (interrupts all
          ...
          if (speechItem.mParams != null) {
            for (int i = 0; i < speechItem.mParams.size() - 1; i = i + 2) {
              ...
              if (param != null) {
                ...
                } else if (param.equals(TextToSpeechBeta.Engine.KEY_PARAM_UTTERANCE_ID)) {
                  utteranceId = speechItem.mParams.get(i + 1);
  ...
  private final ITtsBeta.Stub mBinder = new ITtsBeta.Stub() {
     ...
     *        all previous utterances) TextToSpeech.TTS_QUEUE_ADD for queued
     * @param params An ArrayList of parameters. The first element of this array
     *        controls the type of voice to use.
     */
    ...
    /**
     * Sets the pitch for the TTS. Note that this will only have an effect on
     * synthesized speech; it will not affect pre-recorded speech.
     * 
     ...
     * @param variant the variant code associated with the country and language
     *        pair.
     ...
     * 
     * @return null is no language is set, or an array of 3 string containing
     *         respectively the language, country and variant.
     */
