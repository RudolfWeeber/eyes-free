...
public class TextToSpeechBeta extends TextToSpeech {
   ...
   * to this method, subsequent calls to {@link #speak(String, int, HashMap)}
   ...
   *        where the key can be {@link Engine#KEY_PARAM_STREAM} or
   ...
   * mode.
   * 
   * @param durationInMs A long that indicates how long the silence should last.
   * @param queueMode {@link #QUEUE_ADD} or {@link #QUEUE_FLUSH}.
   * @param params The list of parameters to be used. Can be null if no
   *        parameters are given. They are specified using a (key, value) pair,
   *        where the key can be {@link Engine#KEY_PARAM_UTTERANCE_ID}.
   * 
   * @return Code indicating success or failure. See {@link #ERROR} and
   *         {@link #SUCCESS}.
   */
    ...
    synchronized (mStartLock) {
      ...
      try {
        if (speechRate > 0) {
          ...
          // the rate is not set here, instead it is cached so it will be
          // associated
   ...
   * This has no effect on any pre-recorded speech.
   * 
   * @param pitch The pitch for the TextToSpeech engine. 1 is the normal pitch,
  ...
  /**
   * Sets the language for the TextToSpeech engine. The TextToSpeech engine will
   ...
   * used. Use {@link #isLanguageAvailable(Locale)} to check the level of
   ...
   *         {@link #LANG_COUNTRY_VAR_AVAILABLE}, {@link #LANG_MISSING_DATA} and
  ...
  public int setLanguage(Locale loc) {
    ...
    synchronized (mStartLock) {
      ...
      try {
        ...
        // the language is not set here, instead it is cached so it will be
        // associated
   ...
   * 
   * @return language, country (if any) and variant (if any) used by the engine
