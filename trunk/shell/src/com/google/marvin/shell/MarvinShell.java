...
public class MarvinShell extends Activity implements GestureListener {
	...
	private void loadHomeMenu() {
		...
		items.put(Gesture.UPLEFT, new MenuItem(
		 ...
		 * "LAUNCH", null, compass));
		 */
		items.put(Gesture.RIGHT, new MenuItem("Location", "WIDGET", "LOCATION",
				null));

		AppEntry appLauncher = new AppEntry(null, "com.google.marvin.shell",
				"com.google.marvin.shell.AppLauncher", null, null);
		items.put(Gesture.DOWN, new MenuItem(getString(R.string.applications),
				"LAUNCH", null, appLauncher));

		/*
		 * AppEntry camera = new AppEntry(null, "com.android.camera",
		 * "com.android.camera.Camera", null, null);
		 * items.put(Gesture.DOWNRIGHT, new MenuItem(getString(R.string.camera),
		 * "LAUNCH", null, camera));
		 */

		items.put(Gesture.DOWNRIGHT, new MenuItem("Connectivity", "WIDGET",
				"CONNECTIVITY", null));
		...
		public void run() {
			if (((System.currentTimeMillis() - currentGestureTime) > 250)
				...
				if (item != null) {
					...
					} else if (label.equals(getString(R.string.voicemail))
							&& messageWaiting) {
