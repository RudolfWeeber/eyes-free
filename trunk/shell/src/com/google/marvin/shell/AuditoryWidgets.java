...
public class AuditoryWidgets {
	...
	private String getDataNetworkInfo() {
	...
	public void announceConnectivity() {
		...
		String dataNetworkInfo = getDataNetworkInfo();
		String voiceNetworkInfo = getVoiceNetworkInfo();
		...
		try {
			...
			if ((locationProviders.length() > 0) && locationProviders.contains("gps")){
				gps = parent.getString(R.string.gps_enabled);
			}
		...
		tts.speak(voiceNetworkInfo + " " + dataNetworkInfo + " " + bluetooth + " " + gps, 0, null);
	}
	...
	public void announceDate() {
		...
		try {
			boolean canSpeak = speakingTimeLock.tryLock(1000,
