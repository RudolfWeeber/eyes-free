...
public class Config implements Parcelable {
  ...
  // Only perform text detection, returns bounding boxes
  ...
  private String writeToDisk() {
    ...
    File sdcard = new File(Config.SD_CARD);
...
/*
  private void writeObject(ObjectOutputStream dest) throws IOException {
    ...
    dest.writeObject(bounds);
    dest.writeObject(variables);

    dest.writeObject(language);
  ...
  private void readObject(ObjectInputStream src) throws IOException, ClassNotFoundException {
    ...
    if (length > DISK_THRESHOLD) {
      String file = (String)src.readObject();
      readFromDisk(file);
    } else {
    ...
    bounds = (ArrayList<Rect>)src.readObject();

    variables = (HashMap<String, String>)src.readObject();

    language = (String)src.readObject();
